...
configure_flags="--with-openssl --without-libxml2 --enable-static --disable-shared"
...
if [ "$arch" == "arm64" ] || [ "$arch" == "aarch64" ]; then
    ...
    sudo apt-get install -y libssl-dev:arm64 libexpat1-dev:arm64 zlib1g-dev:arm64 \
        libcppunit-dev:arm64 libxml2-dev:arm64
    ...
    export CPPFLAGS="-I/usr/include/aarch64-linux-gnu"
    export LDFLAGS="-L/usr/lib/aarch64-linux-gnu"

    configure_flags="$configure_flags --host=aarch64-linux-gnu --build=x86_64-linux-gnu"
    ...
    echo "Setting up native compilation for x64..."
